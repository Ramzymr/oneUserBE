...
module.exports.root = function (req, res) {
    ...
    if (!req.session.user || req.session.user == undefined) {
        ...
        console.log("Session user in / is :" + req.session.user)
...
module.exports.login = function (req, res) {
    ...
    if (!req.body.username || !req.body.password) {
        ...
        var isValidUser = vs.isValidUser(userid, pwd, function (result) {
...
module.exports.logout = function (req, res) {
    ...
    console.log("logout get: session user id" + req.session.userid);
    var log = {
        email: '',
        application: 0,
        datetime: '',
    ...
    if (!req.session.user || req.session.user == undefined) {
        ...
        log.email = req.session.userid;
        log.token = req.sessionID;
        var now = new Date();
        log.datetime = now;
        log.message = "Successful";
        lg.log(log);
...
module.exports.displayLogs = function (req, res) {
    console.log("logs: session user" + req.session.user);
    var perPage = 10;
    if (!req.session.user || req.session.user == undefined) {
        console.log("Session user is:" + req.session.user)
        ...
        if (req.body.isFirst == undefined) {
            ...
            if (date1 != "" && date2 == "") {
                sql = sql + " WHERE log_datetime >= '" + date1 + " 00:00:00'";
                //sql = sql+" WHERE (log_datetime >='"+date1 +" 00:00:00' AND <='"+date2+" 23:59:59')";
                msg = "[Date range: " + date1 + " - " + "todate" + "]"
            }
            if (date1 == "" && date2 != "") {
                sql = sql + " WHERE log_datetime <= '" + date2 + " 23:59:59'";
                //sql = sql+" WHERE (log_datetime >='"+date1 +" 00:00:00' AND <='"+date2+" 23:59:59')";
                msg = "[Date range: upto " + date2 + "]"
            }
            if (userInList != "") {
